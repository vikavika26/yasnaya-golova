...
ok(trig.includes('просто совпадение'), 'статус «просто совпадение» вместо «не подтвердился»');
...
ok(trig.includes('Откуда берутся эти выводы'), 'блок с объяснением метода есть');
ok(trig.includes('список подозреваемых'), 'объяснение написано словами');
